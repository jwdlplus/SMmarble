...
void printPlayerStatus(void)
{
     ...
     {       
         printf("%s - position: %i(%s), credit: %i, energy: %i\n",
                    smm_players[i].name, smm_players[i].pos, smmObj_getObjectName(smmdb_getData(LISTNO_NODE, smm_players[i].pos)),
...
void generatePlayers(int n, int initEnergy) //generate a new player
{
     ...
     for (i=0;i<n;i++)
     {
         ...
         smm_players[i].experiment_success = 0;
    ...
    switch(type)
    {
        ...
        case SMMNODE_TYPE_LABORATORY:
             ...
             if (smm_players[player].flag_experiment == 1)
             {
                 ...
                 if (dice >= smm_players[player].experiment_success)
                 {
                          printf("Experiment Success! (Dice: %d, Success point: %d\n",
                                             dice, smm_players[player].experiment_success);
                          smm_players[player].flag_experiment = 0;
                 }
                 else
                     printf("Experiment Failed. Dice: %d, Success point: %d\n",
                                             dice, smm_players[player].experiment_success);
             }
             break;
        ...
        case SMMNODE_TYPE_HOME:
             ...
             if (smm_players[player].credit >= GRADUATE_CREDIT)
             {
                  ...
                  printf("%s satisfied the graduation requirements!\n", smm_players[player].name);
                  int size = smmdb_len(LISTNO_OFFSET_GRADE+player);
                  int i;
                  for (i=0;i<size;i++)
                  {
                      void *ptr = smmdb_getData(LISTNO_OFFSET_GRADE+player, i);
                      printf("Name: %s, Credit: %d, Grade: %s\n",
                                    smmObj_getObjectName(ptr),
                                    smmObj_getObjectCredit(ptr), 
                                    smmObj_getGradeName(ptr));
                  }
             }
             break;
        ...
        case SMMNODE_TYPE_GOTOLAB:
             ...
             smm_players[player].experiment_success = rand()%MAX_DIE + 1;
             printf("Experiment Start! Success point: %d\n", smm_players[player].experiment_success);
             break;
             
        case SMMNODE_TYPE_FOODCHANCE:
        {
             int i = rand()%smm_food_nr;
             void *food = smmdb_getData(LISTNO_FOODCARD, i);
